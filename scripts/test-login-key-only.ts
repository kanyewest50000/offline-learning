...
// gate copy lives in shrine/markup.js, the login handler in shrine/chat.js
...
must(shrine.includes("already a member? log in with your key."), "shrine login hint must be key-only");
must(!shrine.includes("log in with your username and key."), "shrine login must not ask for a username");
must(!shrine.includes('placeholder="username"'), "shrine login box must not have a username field");
must(shrine.includes('placeholder="desired username"'), "apply form still asks for a username");
must(shrine.includes('apiPost("/login",{token:k})'), "shrine login posts the key only");
must(!shrine.includes("enter your username and key."), "shrine login error must not mention username");
